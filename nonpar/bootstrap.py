...
spearman_rs = [stats.spearmanr(df[col].to_numpy(), blackbox).statistic for col in educator_cols]
print(f"Median Spearman r across educators: {np.median(spearman_rs):.3f}")

# Bootstrap 95% CI for median Spearman r
rng = np.random.default_rng(42)
n = len(spearman_rs)
boot_medians = [np.median(rng.choice(spearman_rs, size=n, replace=True)) for _ in range(1000)]
