...
-   Assume a set of [%g event "events" %] \\(E\\) and a probability function \\(P\\) with \\(0 \leq P(x) \leq 1\\)
...
-   The [%g conditional_probability "conditional probability" %] \\(P(A \mid B)\\) of \\(A\\) given \\(B\\) is
    the probability of \\(A\\) occurring given that \\(B\\) is known to occur
-   \\(P(A \mid B) = \frac{P(A \cap B)}{P(B)}\\)

## Bayes Rule

-   Since \\(P(A \mid B) = P(B \mid A)\\), we have [%g bayes_rule "Bayes' Rule" %]
...
-   The [%g mean "mean" %] \\(\mu\\) or [%g expected_value "expected value" %] \\(E(X)\\) is the weighted sum of possible outcomes
    \\(\sum_{x} xP(x)\\)
-   Implies \\(E(aX + bY + c) = aE(X) + bE(Y) + c\\)
-   The [%g variance "variance" %] \\(\sigma^2\\) is the expected value of the square of the difference between values and the mean
    \\(\sum_{x} (x - \mu)^2P(x)\\)
...
-   The [%g standard_deviation "standard deviation" %] \\(\sigma\\) is the square root of the variance
    -   Same units as original variable

## Covariance and correlation

-   [%g covariance "Covariance" %] \\(\sigma_{XY}\\) of \\(X\\) and \\(Y\\) is \\(E((X - \mu_{X})(Y - \mu_{Y}))\\)
...
-   [%g pearson_correlation_coefficient "Pearson's correlation coefficient" %] \\(r_{XY}\\) is covariance normalized by standard deviations
    -   \\(r_{XY} = \frac{\sigma_{XY}}{\sigma_X \sigma_Y}\\)
    -   Always lies in \\([-1 \ldots 1]\\)
-   [%g chebyshev_inequality "Chebyshev's Inequality" %]: \\(P(\mid X - \mu \mid \gt \epsilon) \leq (\frac{\sigma}{\epsilon})^2\\)
...
-   A [%g bernoulli_distribution "Bernoulli distribution" %] is
...
-   A [%g binomial_distribution "binomial distribution" %] is
...
-   A [%g geometric_distribution "geometric distribution" %] is
...
-   The [%g negative_binomial_distribution "negative binomial distribution" %] is
...
-   Number of events occurring within a fixed period has a [%g poisson_distribution "Poisson distribution" %]
...
-   Function describing probabilities of discrete events is called the [%g pmf "probability mass function" %]
-   When describing continuous events, use:
    -   [%g cdf "Cumulative distribution function" %] \\(F(x) = P(X \leq x)\\)
    -   [%g pdf "Probability density function" %] \\(f(x) = dF/dx\\)
...
-   [%g uniform_distribution "Uniform distribution" %] has equal probability over a finite range \\([a \ldots b]\\)
-   \\(f(x) = \frac{1}{b - a}\\)
-   \\(P(a \leq t \leq X \leq t+h \leq b) = \frac{h}{b - a}\\)
    -   I.e., probability is proportional to fraction of range
-   [%g standard_uniform "Standard uniform distribution" %] has range \\([0 \ldots 1]\\)
    -   \\(\mu = \frac{1}{2}\\)
...
-   If there are \\(\alpha\\) independent sequential steps,
    ...
    then the total time has a [%g gamma_distribution "Gamma distribution" %] Gamma(\\(\alpha\\), \\(\lambda\\))
...
-   In its full glory, [%g normal_distribution "normal distribution" %]
...
-   The [%g standard_normal "standard normal distribution" %] \\(Z\\) has mean \\(\mu = 0\\) and standard deviation \\(\sigma = 1\\)
...
-   In particular, binomial(\\(n\\), \\(p\\)) \\(\approx\\) normal(\\(\mu = np\\), \\(\sigma = \sqrt{n p (1 - p)}\\) )
    -   But use a [%g continuity_correction "continuity correction" %]:
...
-   A [%g population "population" %] has a parameter; a [%g sample "sample" %] has a [%g statistic "statistic" %]
...
-   The [%g median "median" %] is the central value such that \\(P(X \lt M) \leq 0.5\\) and \\(P(X \gt M) \leq 0.5\\)
    -   If the distribution is skewed to the right, \\(M \lt \mu\\)
    -   If the distribution is skewed to the left, \\(M \gt \mu\\)
-   A [%g quartile "quartile" %] is a value that divides the sample into quarters
    ...
    -   The [%g iqr "interquartile range" %] IQR is \\(Q_3 - Q_1\\)
...
-   The [%g sample_variance "sample variance" %] is:
...
-   Using \\(n-1\\) instead of \\(n\\) ensures that \\(s^2\\) is unbiased (the [%g bessel_correction "Bessel correction" %])
    -   See [proof](#bessel-correction)

## Parameter estimation

-   The \\(k^{th}\\) [%g population_moment "population moment" %] is \\(E(X^k)\\)
-   The \\(k^{th}\\) [%g sample_moment "sample moment" %] is \\(\frac{1}{n}\sum_{i=1}^{n} X_i^k\\)
    -   The first sample moment is the sample mean
-   The population and sample [%g central_moment "central moments" %] are \\(E(X - \mu)^k\\) and \\(\frac{1}{n}\sum(X_i - \bar{X})^k\\) respectively
    -   I.e., the moments after shifting the data to a mean of zero
-   Parameter estimation via the [%g method_of_moments "method of moments" %]:
    find parameter values to match sample moments
-   Parameter estimation via [%g maximum_likelihood_estimation "maximum likelihood" %]:
...
-   [%g t_distribution "Student's *t*-distribution" %] is used to estimate the mean of a normally distributed population
...
-   However, the variable \\(\frac{\bar{X} - \mu}{s / \sqrt{n}}\\) has a *t*-distribution
    with \\(n-1\\) [%g degrees_of_freedom "degrees of freedom" %]
...
-   A [%g confidence_interval "confidence interval" %] is an interval \\([a \ldots b]\\)
...
-   If there are more than 30 samples or the standard deviation \\(\sigma\\) is known, use a [%g z_test "z-test" %]:
...
-   If there are fewer than 30 samples or the standard deviation isn't known, use a [%g t_test "t-test" %]:
...
-   What is the probability of seeing this difference between two datasets?
    -   The [%g null_hypothesis "null hypothesis" %] \\(H_0\\) is that the samples come from a single population
        and the observed difference is purely due to chance
    -   The [%g alternative_hypothesis "alternative hypothesis" %] \\(H_A\\) is that
        the samples come from two difference populations
    -   [%g false_positive "False positive" %]: decide that the difference is not purely random when it is
    -   [%g false_negative "False negative" %]: decide the difference is purely random when it isn't
...
-   [%g accuracy "Accuracy" %] is the fraction of correct predictions (true positive + true negative)
    -   Not useful if there are only a few defective items, since "all good" will have high accuracy
-   [%g precision "Precision" %] is the fraction of positives that are actually positive,
    i.e. true positive / (true positive + false positive)
-   [%g recall "Recall" %] is the fraction of positives the method can actually identify,
...
-   The [%g f_measure "F-measure" %] is the [%g harmonic_mean "harmonic mean" %] of precision and recall

## Spearman's rank correlation

-   [%g spearmans_rank_correlation "Spearman's rank correlation" %] measures the [%g rank_correlation "rank correlation" %] between two variables
...
where \\(\Gamma\\) is the [%g gamma_function "gamma function" %].
